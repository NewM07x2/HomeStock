...
import { fetchYearlyUsage } from '@/lib/api'
...
export default function YearlyUsageChart() {
  const [data, setData] = useState<YearlyUsage[]>([])
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true)
        setError(null)
        const usage = await fetchYearlyUsage(selectedYear)
        if (!mounted) return
        setData(usage)
        setVisibleCount(5) // データ更新時に表示件数をリセット
      } catch (err) {
        console.error('年別利用金額の取得に失敗しました:', err)
        if (mounted) {
    ...
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
          ...
          >
            {generateYearOptions().map(year => (
              <option key={year} value={year}>{year}年基準</option>
            ))}
          </select>
